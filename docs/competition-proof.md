...
| **Bitget MCP Market Adapter** | **PROTOCOL DISCOVERY / FAIL-CLOSED** | `BitgetMcpMarketDataProvider` targeting `https://agent.bitget.com/mcp`. Uses dynamic `tools/list` protocol discovery. Bitget MCP verification pending/unavailable — no MCP market data used. Resolution order: 1. Bitget MCP (if tool confirmed) → 2. Bitget Spot API → 3. Stooq Reference Fallback → 4. Fail-closed. |
...
| **Market Data Adapter** | **MULTI-MODE** | Real-time Bitget MCP/Spot/Stooq pipeline for live execution + `MockMarketDataProvider` for offline demo scenario simulations. |
